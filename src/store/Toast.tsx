...
export function ToastContainer() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 space-y-2 w-[90vw] max-w-sm">
      {toasts.map((t) => (
        <div key={t.id} className={`rounded-md shadow-lg border px-4 py-3 text-sm bg-white ${
          ...
          <button onClick={() => dismissToast(t.id)} className="text-xs text-gray-500 hover:text-gray-700 mt-1">Dismiss</button>
        </div>
      ))}
    </div>
  );
